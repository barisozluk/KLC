...
{
    /// <summary>
    /// Interaction logic for DeleteUcBirimArayuzPopupWindow.xaml
    /// </summary>
    public partial class DeleteUcBirimArayuzPopupWindow : Window
    {
        bool agArayuzuMu;
        bool girdiMi;
        bool ciktiMi;
        bool gucArayuzuMu;
        public DeleteUcBirimArayuzPopupWindow(bool agArayuzuMu, bool girdiMi, bool ciktiMi, bool gucArayuzuMu)
        {
        ...
        {
            var owner = Owner as UcBirimPopupWindow;

            if(gucArayuzuMu)
            {
                owner.gucArayuzuList.Clear();
                owner.UcBirimGucArayuzDataGrid.ItemsSource = null;
                owner.UcBirimGucArayuzDataGrid.ItemsSource = owner.gucArayuzuList;
                owner.UcBirimGucArayuzDataGrid.Visibility = Visibility.Hidden;
            ...
            if (agArayuzuMu)
            {
                ...
                owner.UcBirimAgArayuzDataGrid.ItemsSource = null;
                owner.UcBirimAgArayuzDataGrid.ItemsSource = owner.agArayuzuList;

                if (owner.agArayuzuList != null && owner.agArayuzuList.Count() > 0)
                {
                    owner.UcBirimAgArayuzDataGrid.Visibility = Visibility.Visible;
                    owner.AgArayuzuNoDataRow.Visibility = Visibility.Hidden;
                }
                else
                {
                    owner.UcBirimAgArayuzDataGrid.Visibility = Visibility.Hidden;
